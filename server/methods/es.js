...
import ValidateContribution from "/lib/modules/cdr/validate_contribution.js";
import { versions } from "/lib/configs/cdr/data_models";
import { levels } from "/lib/configs/cdr/search_levels.js";
  ...
  Meteor.methods({
    async esBuckets({ index, type, queries, filters, aggs }) {
      ...
      try {
        ...
        if (
          !_.find(_.map(queries, "terms"), "summary.contribution._private_key")
        )
              ...
              esClient.bulk(
                {
                  body: bulkIndexChunk.map((row) => {
                    if (row && row.summarizer) {
                      indexType = row.summarizer.indexType;
                      row =
                        summarizer.json[row.summarizer.indexType][
                          row.summarizer.name
                ...
                (err, resp) => {
                  ...
                  if (!resp || resp.body.errors) {
                    console.error(
                      ...
                      "of",
                      n,
                      JSON.stringify(resp.body.items)
                    );
                    resolve(false);
                  } else {
                    console.log(
      ...
      try {
        ...
        if (isLatest === "true") {
          history.slice(1).forEach((v) => {
            try {
              s3DeleteKeys({
                bucket: "cdr-contributions",
                keys: [`${v.id}/cdr_contribution_${v.id}.txt`],
              }).then(() => {
                console.log(
                  "esUploadActivatedContributionToS3",
                  `Deleted ${v.id}/cdr_contribution_${v.id}.txt`
                );
                s3DeleteKeys({
                  bucket: "cdr-contributions",
                  keys: [`${v.id}/`],
          ...
          try {
            s3UploadObject({
              bucket: "cdr-contributions",
              key: `${id}/cdr_contribution_${id}.txt`,
              body: contributionText,
            }).then(() => {
              console.log(
                "esUploadActivatedContributionToS3",
                `Uploaded ${id}/cdr_contribution_${id}.txt to cdr-contributions`
              );
            });
          } catch (e) {
            console.log(
        ...
        try {
          s3UploadObject({
            bucket: "cdr-activated-contributions",
            key: `${id}/cdr_contribution_${id}.txt`,
            body: contributionText,
          }).then(() => {
            console.log(
              "esUploadActivatedContributionToS3",
              `Uploaded ${id}/cdr_contribution_${id}.txt to cdr-activated-contributions`
            );
          });
        } catch (e) {
          console.log(
    ...
    async esGetMeasurements({ index, cid }) {
      console.log("esGetMeasurements", index, cid);
      this.unblock();
      try {
        // Query using the searchable summary data structure
        // Since contribution field is not indexed, we need to get the full contribution data
        let resp = await esClient.search({
          index: index,
          type: "_doc",
          body: {
