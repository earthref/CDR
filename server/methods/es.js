      ...
      try {
        ...
        if (isLatest === 'true') {
          history.slice(1).forEach(v => {
            try {
              s3DeleteKeys({
                ...
                console.log("esUploadActivatedContributionToS3", `Deleted ${v.id}/cdr_contribution_${v.id}.txt`);
                s3DeleteKeys({
                  bucket: 'cdr-contributions',
                  keys: [`${v.id}/`]
                }).then(() => {
          ...
          try {
            s3UploadObject({
              ...
              console.log("esUploadActivatedContributionToS3", `Uploaded ${id}/cdr_contribution_${id}.txt to cdr-contributions`);
            });
          } catch (e) {
        ...
        try {
          s3UploadObject({
            ...
            console.log("esUploadActivatedContributionToS3", `Uploaded ${id}/cdr_contribution_${id}.txt to cdr-activated-contributions`);
          });
        } catch (e) {
