...
// Split on colons that delimit entries rather than colons within a value, so URLs
// survive (e.g. 'OSU-MGR["https://osu-mgr.org/OSU-OD1507-18GC"]'). Skips colons inside
// the [value] brackets and, matching validate_contribution, colons inside quotes.
let reDictionaryDelimiter = new RegExp(/:(?![^\[]*\])(?=(?:[^"]*"[^"]*")*[^"]*$)/);
// Values containing colons are wrapped in quotes on export (see escapeColons)
let reWrappingQuotes = new RegExp(/^"(.*)"$/);
...
export default class extends Runner {
  ...
  _summarizeTables(onlyCounts) {
    ...
    return Promise.each(sortedTables, (table, tableIdx) => {
      ...
      return new Promise((resolve) => {
        ...
        // console.log('summarizing', table, this.contribution[table]);
        ...
        if (table === 'measurements') {
          _.keys(this.json[table]).forEach((tableProp) => {
            _.keys(this.json[table][tableProp]).forEach((parentProp) => {
              let experimentProp = parentProp;
              if (this.json.experiments) _.keys(this.json.experiments[experimentProp]).forEach((sectionProp) => {
                this._aggregateSummaries('experiments',
                  this.json.experiments[experimentProp][sectionProp],
                  this.json[table][tableProp][parentProp]
                );
                if (this.json.sections) _.keys(this.json.sections[sectionProp]).forEach((coreProp) => {
                  this._aggregateSummaries('sections',
                    this.json.sections[sectionProp][coreProp],
                    this.json[table][tableProp][parentProp]
                  );
                  if (this.json.cores) _.keys(this.json.cores[coreProp]).forEach((cruiseProp) => {
                    this._aggregateSummaries('cores',
                      this.json.cores[coreProp][cruiseProp],
                      this.json[table][tableProp][parentProp]
                    );
                    if (this.json.cruises) _.keys(this.json.cruises[cruiseProp]).forEach((locationProp) => {
                      this._aggregateSummaries('cruises',
                        this.json.cruises[cruiseProp][locationProp],
                        this.json[table][tableProp][parentProp]
                      );
                    });
                  });
  ...
  _adoptChildTables(toTable, onlyCounts) {
    ...
    return Promise.each(sortedTables, (table) => {
      ...
      return new Promise((resolve) => {
        ...
        if (table === 'cruises') {
          _.keys(this.json.cruises).forEach((cruiseProp) => {
            _.keys(this.json.cruises[cruiseProp]).forEach((prop) => {
              if (!toTable || toTable === 'contribution') {
                this._aggregateSummaries(table,
                  this.json[table][cruiseProp][prop],
        ...
        if (table === 'cores') {
          _.keys(this.json.cores).forEach((coreProp) => {
            _.keys(this.json.cores[coreProp]).forEach((cruiseProp) => {
              if (!toTable || toTable === 'contribution') {
                this._aggregateSummaries(table,
                  this.json[table][coreProp][cruiseProp],
                  this.json.contribution, undefined, onlyCounts
                );
              }
              if (this.json.cruises) _.keys(this.json.cruises[cruiseProp]).forEach((prop) => {
                if (!toTable || toTable === 'cruises') {
                  this._aggregateSummaries(table,
                    this.json[table][coreProp][cruiseProp],
                    this.json.cruises[cruiseProp][prop], undefined, onlyCounts
                  );
                }
              });
            });
        ...
        if (table === 'sections') {
          _.keys(this.json.sections).forEach((sectionProp) => {
            _.keys(this.json.sections[sectionProp]).forEach((coreProp) => {
              if (!toTable || toTable === 'contribution') {
                this._aggregateSummaries(table,
                  this.json[table][sectionProp][coreProp],
                  this.json.contribution, undefined, onlyCounts
                );
              }
              if (this.json.cores) _.keys(this.json.cores[coreProp]).forEach((cruiseProp) => {
                if (!toTable || toTable === 'cores') {
                  this._aggregateSummaries(table,
                    this.json[table][sectionProp][coreProp],
                    this.json.cores[coreProp][cruiseProp], undefined, onlyCounts
                  );
                }
                if (this.json.cruises) _.keys(this.json.cruises[cruiseProp]).forEach((prop) => {
                  if (!toTable || toTable === 'cruises') {
        ...
        if (table === 'experiments') {
          _.keys(this.json.experiments).forEach((experimentProp) => {
            _.keys(this.json.experiments[experimentProp]).forEach((sectionProp) => {
              if (!toTable || toTable === 'contribution') {
                this._aggregateSummaries(table,
                  this.json[table][experimentProp][sectionProp],
                  this.json.contribution, undefined, onlyCounts
                );
              }
              if (this.json.sections) _.keys(this.json.sections[sectionProp]).forEach((coreProp) => {
                if (!toTable || toTable === 'sections') {
                  this._aggregateSummaries(table,
                    this.json[table][experimentProp][sectionProp],
                    this.json.sections[sectionProp][coreProp], undefined, onlyCounts
                  );
                }
                if (this.json.cores) _.keys(this.json.cores[coreProp]).forEach((cruiseProp) => {
                  if (!toTable || toTable === 'cores') {
          ...
          _.keys(this.json.measurements).forEach((measurementProp) => {
            _.keys(this.json.measurements[measurementProp]).forEach((experimentProp) => {
              if (!toTable || toTable === 'contribution') {
                this._aggregateSummaries(table,
                  this.json[table][measurementProp][experimentProp],
                  this.json.contribution, undefined, onlyCounts
                );
              }
              if (this.json.experiments) _.keys(this.json.experiments[experimentProp]).forEach((sectionProp) => {
                if (!toTable || toTable === 'experiments') {
                  this._aggregateSummaries(table,
                    this.json[table][measurementProp][experimentProp],
                    this.json.experiments[experimentProp][sectionProp], undefined, onlyCounts
                  );
                }
                if (this.json.sections) _.keys(this.json.sections[sectionProp]).forEach((coreProp) => {
                  if (!toTable || toTable === 'sections') {
  ...
  _summarizeRowProps(row, summary, model) {
    ...
    if (row.age_unit !== undefined && !isNaN(parseFloat(row.age_one_sigma))) {
      let age_sigma = parseFloat(row.age_one_sigma);
      if (!isNaN(age_sigma)) {
        let age_sigma_y;
        ...
        if (age_sigma_y !== undefined) {
          this._summerizeNumber(summary, '_age_one_sigma_y', age_sigma_y);
        }
      }
    }

    if (row.age_unit !== undefined && !isNaN(parseFloat(row.age_one_sigma_stderr))) {
      let age_sigma = parseFloat(row.age_one_sigma_stderr);
      if (!isNaN(age_sigma)) {
        let age_sigma_y;
        if (row.age_unit === 'Ga'                ) age_sigma_y = 1e9*age_sigma;
        if (row.age_unit === 'Ma'                ) age_sigma_y = 1e6*age_sigma;
    ...
    _.keys(row).forEach((column) => {
      if (!model.columns[column]) {
        ...
        if (model.columns[column].type === 'List') {
          ...
          if (row[column] && row[column].split) row[column].split(reDictionaryDelimiter).forEach((val) => {
            let match = val.match(reDictionary);
            if (match && match.length > 2) {
              this._initProp(summary, column, []);
              if (summary[column].length < maxValsArrayLength)
                summary[column].push({
                  key: match[1].replace(reWrappingQuotes, '$1'),
                  value: match[2].replace(reWrappingQuotes, '$1')
                });
            }
          });
        } else {
