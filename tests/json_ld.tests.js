        ...
        if (resp.hits.total > 0) {
          xml = resp.hits.hits.forEach(hit => {
            hit._source.summary.contribution._history.forEach(contribution => {
              ...
              `<loc>https://earthref.org/CDR/${contribution.id}</loc>` + 
              `<lastmod>${contribution.timestamp}</lastmod>` +
            `</url>`;
            });
          });
        }
        fs.writeFileSync(`C:/Users/rminn/source/repos/CDR/public/CDR/contributions.sitemap.xml`, 
