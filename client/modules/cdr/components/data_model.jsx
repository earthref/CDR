...
import DataModelColumn from '/client/modules/cdr/components/data_model_column';
  ...
  search(str) {
    ...
    if (str !== '') {
      ...
      $tbls.find('.data-model-table-count').each(function() {
        ...
        $(this).addClass(portals['CDR'].color);
      ...
      $(this.refs['count']).addClass(portals['CDR'].color);
    }
    ...
    // Enable all columns since the search string is empty.
      ...
      $counts.removeClass(portals['CDR'].color);
      $counts.children().remove();
      $(this.refs['count']).html($cols.length);
      $(this.refs['count']).removeClass(portals['CDR'].color);
    }
