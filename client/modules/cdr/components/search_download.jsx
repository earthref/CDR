...
export default class extends React.Component {
  ...
  downloadLevelRows() {
    ...
    this.canceled = false;
    this.downloadedRows = {contribution: [{data_model_version: _.last(versions), description: `Downloaded from CDR at ${moment().utc().toISOString()}.`}]};
    let processedHits = 0;
      ...
      try {
        if (error) {
          ...
          if (results.hits.total.value > processedHits)
            ...
            saveAs(blob, 'cdr_downloaded_rows.txt');
          } else {
            const exporter = new ExportContribution({});
            ...
            saveAs(workbookBlob, 'cdr_downloaded_rows.xlsx');
          }
        }
      } catch (error) {
        console.error(error);
      ...
      >
        ...
        { this.state.renderModal && <div ref="download modal" className="ui modal">
          ...
          <div className="content">
            <table style={{margin: "auto"}}>
              <tbody>
                ...
                <tr>
                  <td colSpan="3">
                    <Checkbox type="radio" name="format" value="text" label="CDR Text File"
            ...
            { this.state.progress !== undefined &&
              <Progress percent={this.state.progress} color="yellow" progress autoSuccess active={this.state.progress < 100}
                style={{margin:0}}
              />
            }
          </div>
          ...
          { this.state.downloadContributions && (this.state.progress === undefined || this.state.progress === 100) &&
              <div className="ui button yellow" onClick={this.downloadContributionFiles.bind(this)}>
                Prepare Files
              </div>
            }
            { this.state.downloadContributions && (this.state.downloadIDs.length > 0 && this.state.progress === 100) &&
              <button type="submit" className="ui button yellow" onClick={function (e) {
                Meteor.call('cdrGetPublicContributions', this.state.downloadIDs, 'cdr_search_results.zip', '@' + Cookies.get('user_id', Meteor.isDevelopment ? {} : { domain: '.earthref.org'}), function (error, source) {
                  if (source) {
                    let blob = new Blob([source], {type: "application/zip"});
                    saveAs(blob, 'cdr_search_results.zip');
                  } else {
                    console.error(error);
            ...
            { !this.state.downloadContributions && (this.state.progress === undefined || this.state.progress === 100) &&
              <div className="ui button yellow" onClick={this.downloadLevelRows.bind(this)}>
                Download Rows
              </div>
            }
